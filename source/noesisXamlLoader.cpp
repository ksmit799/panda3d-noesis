...
Noesis::Ptr<Noesis::Stream> NoesisXamlLoader::LoadXaml(const Noesis::Uri &uri) {
  ...
  if (vf == nullptr) {
    ...
    noesis_cat.warning() << "Could not find XAML file: " << filename << "\n";
    return nullptr;
  }

  std::istream *source = vf->open_read_file(true);
  if (source == nullptr) {
    noesis_cat.warning() << "Could not open XAML file: " << filename << "\n";
    return nullptr;
  }
